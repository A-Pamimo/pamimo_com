  ...
  return (
    <footer className="bg-ink dark:bg-black text-cream py-24 px-4 md:px-12 relative overflow-hidden transition-colors">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
        <div>
          <h2 className="font-display font-extrabold text-display-1 mb-6">READY TO<br /><span className="text-transparent" style={{ WebkitTextStroke: '1px #fff', cursor: 'pointer' }}>BUILD.</span></h2>
          ...
          <div className="mt-12 max-w-sm">
            <p className="font-mono text-xs uppercase tracking-widest mb-2 opacity-70">Or keep in touch</p>
            <form className="relative group" onSubmit={handleSubscribe}>
              <input
                ...
                className={`w-full bg-transparent border-b py-3 text-xs font-mono placeholder:text-cream/50 focus:outline-none transition-colors pr-10 text-cream
                    ${status === 'error' ? 'border-red-500 text-red-500' : 'border-cream/40 focus:border-pop'}
              ...
              <button
                ...
                className="absolute right-0 top-2 opacity-70 group-hover:opacity-100 hover:text-pop transition-all disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pop"
              >
                {status === 'loading' ? (
