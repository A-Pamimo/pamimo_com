...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onClick, className }) => {
  ...
  if (project.id === 'nova') {
    return (
      <div onClick={() => onClick(project)} className={`project-card ${className} border border-ink dark:border-white/20 p-0 group hover:shadow-hard transition-all bg-cream dark:bg-black cursor-hoverable cursor-pointer`}>
  ...
  return (
    <div onClick={() => onClick(project)} className={`project-card ${className} border border-ink dark:border-white/20 p-6 group hover:shadow-hard transition-all cursor-hoverable cursor-pointer flex flex-col justify-between ${project.id === 'city' ? 'bg-pop text-white' : 'bg-white dark:bg-charcoal'}`}>
